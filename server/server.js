      ...
      db.query(
        ...
        (err, output) => {
          if (err) {
            ...
            return res
              .status(500)
              .json({ error: "Error al obtener resultado del registro" });
          }
      ...
      db.query(
        ...
        (err, output) => {
          if (err) {
            ...
            return res
              .status(500)
              .json({ error: "Error al procesar el login" });
          }
          ...
          if (result.resultado === 1) {
            ...
            const statusCode =
              result.mensaje === "Usuario no encontrado"
                ? 400
                : result.mensaje === "El usuario fue eliminado"
                ? 403
                : result.mensaje === "Credenciales inválidas"
                ? 401
                : 500;
          ...
          db.query(
            ...
            (err, output) => {
              if (err) {
                ...
                return res
                  .status(500)
                  .json({ error: "Error al procesar actualización" });
              }
              ...
              if (result.resultado === 1) {
                ...
                const statusCode =
                  result.mensaje === "El correo electrónico ya está en uso"
                    ? 400
                    : result.mensaje === "Usuario no encontrado"
                    ? 404
                    : 500;
      ...
      db.query(
        ...
        (err, output) => {
          if (err) {
            ...
            return res
              .status(500)
              .json({ error: "Error al procesar desactivación" });
          }
          ...
          if (result.resultado === 1) {
            ...
            const statusCode =
              result.mensaje === "Usuario no encontrado"
                ? 404
                : result.mensaje === "El usuario ya estaba desactivado"
                ? 400
                : 500;
      ...
      db.query(
        ...
        (err, output) => {
          if (err) {
            ...
            return res
              .status(500)
              .json({ error: "Error al procesar la película" });
          }
          ...
          if (result.resultado === 1) {
            ...
            const statusCode =
              result.mensaje === "El género especificado no existe"
                ? 400
                : result.mensaje ===
                  "Ya existe una película con este título y director"
                ? 409
                : 500;
...
app.get("/api/peliculas", (req, res) => {
  ...
  db.query("CALL sp_BuscarPeliculas(?)", [searchTerm], (err, results) => {
    if (err) {
  ...
  db.query(
    ...
    (err, results) => {
      ...
      const reseñasFormateadas = reseñas.map((reseña) => ({
        ...reseña,
        autor_avatar: reseña.autor_avatar
          ? `data:image/jpeg;base64,${reseña.autor_avatar}`
          : null,
      }));
...
app.delete("/api/peliculas/:id", (req, res) => {
  ...
  db.query(
    "CALL sp_EliminarPelicula(?, @resultado, @mensaje)",
    [peliculaId],
    (err, results) => {
      if (err) {
...
app.post('/api/modificarPelicula', upload.single('imagen'), (req, res) => {
  const { id, titulo, sinopsis, director, genero, anio } = req.body;
  let imagen = null;
  ...
  if (!id || !titulo || !sinopsis || !director || !genero || !anio) {
    return res.status(400).json({ error: 'Faltan campos obligatorios' });
  }

  db.query(
    'CALL sp_ModificarPelicula(?, ?, ?, ?, ?, ?, ?, @resultado, @mensaje)',
    [id, titulo, sinopsis, director, genero, anio, imagen],
    (err, results) => {
      if (err) {
        console.error('Error al modificar película:', {
          code: err.code,
          errno: err.errno,
          sqlMessage: err.sqlMessage
        });
      ...
      db.query(
        ...
        (err, output) => {
          if (err) {
            ...
            return res
              .status(500)
              .json({ error: "Error al verificar creación de reseña" });
          }
...
app.put("/api/resenias/:id", (req, res) => {
  ...
  if (!comentario || typeof comentario !== "string") {
    return res
      .status(400)
      .json({ error: "El comentario es requerido y debe ser texto" });
  }
      ...
      db.query(
        ...
        (err, output) => {
          if (err) {
            ...
            return res
              .status(500)
              .json({ error: "Error al verificar eliminación" });
          }
      ...
      db.query(
        ...
        (err, output) => {
          ...
          if (resultado === 1) {
            const reseñas = results[0];
            console.log(
              `[GET] Encontradas ${reseñas.length} reseñas para usuario ${usuarioId}`
            );
...
app.post("/api/favoritos", (req, res) => {
  ...
  console.log(
    `[POST] Agregando favorito - Usuario: ${usuario_id}, Película: ${pelicula_id}`
  );
...
app.post("/api/favoritos/check", (req, res) => {
  ...
  console.log(
    `[POST] Verificando favorito - Usuario: ${usuario_id}, Película: ${pelicula_id}`
  );
...
app.delete("/api/favoritos", (req, res) => {
  ...
  console.log(
    `[DELETE] Eliminando favorito - Usuario: ${usuario_id}, Película: ${pelicula_id}`
  );
